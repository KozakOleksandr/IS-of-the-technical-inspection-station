...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Update ": "Create New " ?>Додати пакет</h3>
	</div>
	<div class="card-body">
		<form action="" id="package-form">
			<input type="hidden" name ="id" value="<?php echo isset($id) ? $id : '' ?>">
			<div class="form-group">
				<label for="title" class="control-label">Назва пакета</label>
                <textarea name="title" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($title) ? $title : ''; ?></textarea>
			</div>
            <div class="form-group">
				<label for="tour_location" class="control-label">Розташування туру</label>
            ...
            <div class="form-group">
				<label for="cost" class="control-label">Ціна</label>
            ...
            <div class="form-group">
				<label for="status" class="control-label">Опис</label>
                <select name="status" id="status" class="custom-select selevt">
                <option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Доступна</option>
                <option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Не доступна</option>
                </select>
			</div>
            <div class="form-group">
				<label for="" class="control-label">Зображення</label>
	...
	<div class="card-footer">
		<button class="btn btn-flat btn-primary" form="package-form">Зберегти</button>
		<a class="btn btn-flat btn-default" href="?page=responses">Скасувати</a>
	</div>
</div>
<script>
